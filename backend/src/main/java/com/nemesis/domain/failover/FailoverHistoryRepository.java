...
public interface FailoverHistoryRepository extends JpaRepository<FailoverHistory, Long> {
    ...
    /** 가장 최근 상태별 이력(자동 페일백 후보 판정용). */
    Optional<FailoverHistory> findFirstByClusterGroupIdAndStatusOrderByCreatedAtDesc(
            UUID clusterGroupId, FailoverHistory.Status status);
}
