...
> **레퍼런스 디자인:** 다크 테마 기반 통합 관제 대시보드
> 전체 시스템 상태를 단일 화면에서 확인하고 제어한다.
...
```
┌─────────────────────────────────────────────────────────────────┐
│ 🔷 NEMESIS   대시보드   날짜/시간   🔔 알람   👤 admin           │  ← 상단 헤더
├───────────┬─────────────────────────────────────────────────────┤
│           │  [요약 카드] 전체상태 / 서비스 / HA쌍 / 서버 / 알람  │  ← 요약 카드
│  사이드바  ├──────────────────────────────────┬──────────────────┤
│           │  이중화 동기화 상태 (실시간)        │  주요 서비스 상태  │
│  대시보드  │  Primary ↔ Standby               │  실시간 알람      │
│  서비스    ├──────────┬──────────┬─────────────┤                  │
│  이중화(HA)│  DB 상태  │  SW 상태  │  Docker 상태 │  NEMESIS AI     │
│  서버      ├──────────┴──────────┴─────────────┴──────────────────┤
│  DB 관리   │  진행 중인 작업 (Runbook / 점검) — 단계별 진행 상태   │  ← 하단 작업
│  SW 관리   └─────────────────────────────────────────────────────┘
│  Docker 관리
│  Runbook
│  점검 관리
...
| 카드 | 표시 항목 | 상태 표현 |
|---|---|---|
| 전체 상태 | 시스템 전체 정상/경고/장애 | 아이콘 + 텍스트 (정상/경고/장애) |
| 서비스 | 전체 서비스 수 / 정상 / 경고 / 장애 | 도넛 차트 |
| 이중화(HA) 쌍 | 전체 HA 쌍 수 / 정상 / 장애 | 도넛 차트 |
| 서버 | 전체 서버 수 / 정상 / 장애 | 도넛 차트 |
| 알람 | 치명 / 경고 / 정보 건수 | 도넛 차트 |

---

#### 7-2. 이중화 동기화 상태 패널 (실시간)

```
┌──────────────────────────────────────────────────────────────┐
│  이중화 동기화 상태 (실시간)                          더보기 > │
│                                                              │
│  PRIMARY              실시간 동기화 ✅          STANDBY      │
│  Server01             ●●●●●●●●●●●●●●●          Server02     │
│  192.168.10.11        동기화 상태: SYNC OK      192.168.10.12│
│                       지연 시간(Lag): 0ms                    │
│                       마지막 동기화: 08:30:44                │
│  CPU  23%                                      CPU  21%     │
│  MEM  42%                                      MEM  41%     │
│  DISK 65%                                      DISK 64%     │
│                                                              │
│  가상IP(VIP): 10.10.10.100 [ACTIVE]                         │
│  클러스터 모드: Active / Standby                             │
│  위치: 전산실 본관                                           │
│  마지막 상태 변경: 2025.06.05 07:58:12                      │
│                                    [⚡ 수동 전환(Failover)]  │
└──────────────────────────────────────────────────────────────┘
```

- [ ] Primary / Standby 서버 정보 실시간 표시
- [ ] 동기화 상태 (SYNC OK / LAG / FAIL) 애니메이션 표시
- [ ] 지연 시간(Lag) ms 단위 실시간 표시
- [ ] CPU / MEM / DISK 양쪽 노드 동시 표시
- [ ] VIP 상태 (ACTIVE 보유 노드 강조)
- [ ] 수동 전환(Failover) 버튼 — 클릭 시 확인 다이얼로그
...
│                              │
│  🟢 민원시스템        ● 정상  │
│     Oracle/WebLogic/WAS/Nginx│
│                              │
│  🟢 세무회계시스템    ● 정상  │
│     Oracle/Tomcat/Nginx      │
│                              │
│  🟡 복지시스템        ▲ 경고  │
│     PostgreSQL/WAS/Nginx     │
│                              │
│  🟢 통합예약시스템    ● 정상  │
│     MariaDB/SpringBoot/Nginx │
│                              │
│  🟢 배치시스템        ● 정상  │
│     Job Scheduler/Batch      │
└──────────────────────────────┘
...
- [ ] 서비스(클러스터 그룹)별 전체 상태 한눈에 표시
- [ ] 서비스명 + 구성 SW 스택 표시
- [ ] 정상(초록) / 경고(노랑) / 장애(빨강) 색상 구분
- [ ] 클릭 시 해당 서비스 상세 페이지로 이동
...
│  🔵 Oracle (민원DB)    OPEN   ● 정상 │
│  🔵 Oracle (세무DB)    OPEN   ● 정상 │
│  🟣 PostgreSQL (복지DB) PRIMARY ● 정상│
│  🟠 MariaDB (예약DB)   RUNNING ● 정상│
└──────────────────────────────────────┘
```

- [ ] DB명 / 상태(OPEN/RUNNING/DOWN) / 정상여부 표시
- [ ] DB 타입별 아이콘 구분 (Oracle / PostgreSQL / MariaDB / MySQL)
- [ ] 헬스체크: 프로세스 + 리스너 포트 응답

---

#### 7-5. SW / Middleware 상태 패널

```
┌──────────────────────────────────────┐
│  SW / Middleware 상태         더보기 >│
│                                      │
│  🟦 WebLogic (Admin)   RUNNING ● 정상│
│  🟦 WebLogic (Managed) 2/2    ● 정상 │
│  🟧 Tomcat             3/3    ● 정상 │
│  🟩 Nginx              RUNNING ● 정상│
│  🟥 Redis              RUNNING ● 정상│
└──────────────────────────────────────┘
...
- [ ] SW명 / 인스턴스 수(기동/전체) / 상태 표시
- [ ] 헬스체크: 프로세스 + 포트 + HTTP 응답코드
...
│  Docker 상태                              │
│                                          │
│  Server01 (192.168.10.11)      ● 정상    │
│  컨테이너 8/8  이미지 18                  │
│  동기화: SYNC OK  지연시간: 0ms           │
│                                          │
│  Server02 (192.168.10.12)      ● 정상    │
│  컨테이너 8/8  이미지 18                  │
│  동기화: SYNC OK  지연시간: 0ms           │
└──────────────────────────────────────────┘
```

- [ ] 노드별 컨테이너 기동 수 / 전체 수 표시
- [ ] Docker 이미지 수 표시
- [ ] 컨테이너 동기화 상태 표시
...
│     결과입니다.                          │
│                                          │
│  • 복지시스템 WAS2의 메모리 사용량이      │
│    85% 이상으로 증가 추세입니다.          │
│                                          │
│  • Server03의 디스크 용량이 85%입니다.   │
│    7일 내 임계치 초과 가능성이 있습니다.  │
│                                          │
│  • 민원시스템 DB의 세션 수가 평소보다     │
│    15% 증가했습니다.                     │
│                                          │
│  ┌──────────────────────────┐  [전송 ▶] │
│  │ AI에게 질문하기...        │           │
...
- [ ] AI 실시간 시스템 분석 결과 표시
- [ ] 이상 징후 사전 경고 메시지
- [ ] AI 질문 입력창 (자연어로 시스템 질의)
- [ ] 더보기 클릭 시 AI 상세 분석 페이지 이동

---

#### 7-8. 진행 중인 작업 패널 (Runbook / 점검)

```
┌──────────────────────────────────────────────────────────────┐
│  진행 중인 작업 (Runbook / 점검)                      더보기 >│
│                                                              │
│  🔄 민원시스템 정기점검  [진행 중]                           │
│  시작: 08:20:00  현재: 65% 완료                             │
│                                                              │
│  ✅ Oracle 종료  ✅ WAS 종료  ✅ Docker 종료                 │
│  08:20:10       08:23:15     08:25:40                       │
│                                                              │
│  🔄 Patch 적용   ⏳ Oracle 기동  ⏳ WAS 기동                │
│  진행 중         대기            대기                        │
└──────────────────────────────────────────────────────────────┘
```

- [ ] 현재 실행 중인 Runbook / 점검 작업 실시간 표시
- [ ] 단계별 완료(✅) / 진행중(🔄) / 대기(⏳) / 실패(❌) 상태
- [ ] 전체 진행률 % 및 진행 바 표시
- [ ] 시작 시간 / 단계별 완료 시간 표시
...
│  실시간 알람                      더보기 >│
│                                          │
│  ⚠️ 복지시스템 WAS2 메모리 사용량 경고   │
│     2025.06.05 08:28:31                  │
│                                          │
│  ℹ️ Server03 디스크 사용량 85%           │
│     2025.06.05 08:21:15                  │
│                                          │
│  ℹ️ 배치시스템 일일 배치 완료            │
│     2025.06.05 08:15:02                  │
│                                          │
│  ℹ️ Docker 이미지 업데이트 가능 (3개)    │
│     2025.06.05 07:50:11                  │
└──────────────────────────────────────────┘
```
...
- [ ] 치명(🔴) / 경고(🟡) / 정보(🔵) 3단계 알람 레벨
- [ ] 발생 시간 표시
- [ ] 클릭 시 해당 서비스/노드 상세 이동
- [ ] Telegram / Email 연동 알림 발송
...
| 대시보드 | 전체 현황 Overview |
| 서비스 | 클러스터 그룹별 서비스 관리 |
| 이중화(HA) | HA 쌍 구성 / Failover 설정 |
| 서버 | 노드 목록 / 에이전트 관리 |
| DB 관리 | DB 등록 / 헬스체크 설정 |
| SW 관리 | SW 프로세스 등록 / 모니터링 |
| Docker 관리 | 컨테이너 / 이미지 관리 |
| Runbook | WYSIWYG 절차서 편집 / 실행 |
| 점검 관리 | 정기 점검 스케줄 관리 |
| 리포트 | 장애 이력 / 운영 리포트 |
| 알람 관리 | 알람 정책 / 수신자 설정 |
| 시스템 관리 | 사용자 / 권한 / API Key 관리 |
...
|---|---|
| 테마 | 다크 테마 (Dark) 기본 |
| 주요 색상 | 정상: #00C853 (초록) / 경고: #FFD600 (노랑) / 장애: #FF1744 (빨강) |
| 배경색 | #0D1117 (메인) / #161B22 (카드) |
| 강조색 | #1E88E5 (파랑 — 버튼, 링크) |
| 폰트 | Noto Sans KR (한글) / Inter (영문) |
| 차트 라이브러리 | Chart.js |
| 아이콘 | React Icons / Lucide |
| 실시간 갱신 | WebSocket 또는 SSE (Server-Sent Events) |
...
클러스터 그룹은 메타데이터의 최상위 단위다. 모든 노드/서비스/정책은 그룹에 소속된다.
...
### 7.2 PostgreSQL 핵심 테이블

영구 저장이 필요한 데이터는 PostgreSQL에 저장하고, 실시간 메트릭은 In-Memory 캐시에 보관한다.

| 테이블 | 용도 | 주요 컬럼 |
|---|---|---|
| `cluster_groups` | 클러스터 그룹 정의 (최대 10개) | id, name, description, created_at |
| `nodes` | 노드 정보 | id, group_id(FK), hostname, os, role(active/standby), service_ip, heartbeat_ip, vip |
| `services` | 그룹별 등록 서비스 | id, group_id(FK), name, layer(db/was/web/docker), check_command, order_index |
| `api_keys` | 에이전트 인증 키 | id, group_id(FK), node_id(FK), key_hash, expires_at, revoked_at |
| `users` | 사용자 계정 | id, username, password_hash, role(admin/operator/viewer) |
| `user_group_perms` | 그룹 운영자 ↔ 담당 그룹 매핑 | user_id(FK), group_id(FK) |
| `failover_policies` | 그룹별 Failover/Self-Healing 정책 | group_id(FK), ai_enabled, healing_mode(auto/semi/manual), pingpong_guard_seconds, heartbeat_fail_threshold |
| `runbooks` | 기동/종료/점검 절차서 | id, group_id(FK), name, type(start/stop/check), version, steps(JSONB) |
| `runbook_executions` | Runbook 실행 이력 | id, runbook_id(FK), executed_by, started_at, finished_at, status, step_results(JSONB) |
| `startup_sequences` | WYSIWYG 기동/종료 순서 (F-08) | id, group_id(FK), name, version, steps(JSONB: 순서/대상/명령/타임아웃/실패정책) |
| `events` | 장애/상태 변화 이벤트 | id, group_id(FK), node_id(FK), type, severity, payload(JSONB), created_at |
| `ai_decisions` | AI 판단 기록 | id, event_id(FK), provider, model, input_summary, decision, confidence, latency_ms, created_at |
| `failover_history` | Failover 실행 이력 | id, group_id(FK), from_node, to_node, trigger(ai/manual/heartbeat), duration_ms, result |
| `healing_history` | Self-Healing 실행 이력 | id, group_id(FK), node_id(FK), layer, script, result, retries |
| `alarms` | 알람 발생/확인 이력 | id, group_id(FK), severity, message, acked_by, acked_at |
| `audit_logs` | 감사 로그 (1년 보존) | id, user_id, action, target, detail(JSONB), created_at |

> 메트릭 시계열(CPU/Mem/Disk 3초 주기)은 DB에 raw 저장하지 않고 In-Memory 보관 후
> 1분 단위 집계만 `metrics_rollup` 테이블에 저장한다 (DB 부하 방지).
...
```
cluster_groups 1 ──── N nodes
cluster_groups 1 ──── N services
cluster_groups 1 ──── 1 failover_policies
cluster_groups 1 ──── N runbooks ──── N runbook_executions
cluster_groups 1 ──── N startup_sequences
cluster_groups 1 ──── N events ──── N ai_decisions
cluster_groups 1 ──── N api_keys (1 Key = 1 Node)
users N ──── N cluster_groups (user_group_perms, 그룹 운영자 권한)
```
...
| 장애 감지 시간 | 하트비트 기준 3초 이내, AI 판단 포함 13초 이내 |
| 오탐(False Failover)률 | 월 1건 미만 (핑퐁 방지 정책 적용 기준) |
...
- 클러스터 그룹은 관리 서버 1대당 최대 10개 (MAX_CLUSTER_GROUPS)
- LLM 미연동(또는 LLM 응답 지연/실패) 시에도 하트비트 기반 Rule Failover는 독립 동작해야 함

---

## 11. v1.0 릴리스 범위 (Release Scope)

| 구분 | 포함 (v1.0) | 제외 (로드맵) |
|---|---|---|
| 클러스터 구성 | Active/Standby 1:1, 그룹 최대 10개 | N:M 구성, 그룹 10개 초과 |
| 스토리지 | GPFS | NFS / SAN / DRBD |
| AI | LLM 기반 Failover 판단, Self-Healing (DB/WAS/Web) | 예측 장애 감지(Predictive), 학습 기반 임계값 자동 조정 |
| UI | 통합 대시보드, WYSIWYG 기동/종료 편집기, Runbook | 모바일 전용 UI, 멀티 관리 서버 페더레이션 |
| 알림 | Telegram / Email | Slack / SMS / 전화 발신 |
| 보안 | API Key + TLS, RBAC 3단계 | LDAP/AD 연동, SSO |

> Phase별 일정은 `Nemesis_Roadmap.md` 참조 (Phase 1 Foundation 2026 Q3 → Phase 5 Scale-Out 2027 Q3~)
...
| 포트 | 프로토콜 | 구간 | 용도 |
|---|---|---|---|
| 18080 | HTTP | 에이전트 → 관리 서버 | 메트릭/로그 Push |
| 18443 | HTTPS | 에이전트 → 관리 서버 | 메트릭/로그 Push (TLS) + 메타데이터 Pull |
| 17001 | HTTPS | 관리 서버 → 에이전트 | 제어 명령 (Failover / Self-Healing / 메타데이터 Push) |
| 17000 | TCP | 노드 ↔ 노드 | 하트비트 전용 (1초 주기) |
| 18090 | HTTP/HTTPS | 관리자 → 관리 서버 | Web UI |

## 부록 B — 용어집

| 용어 | 정의 |
|---|---|
| 클러스터 그룹 | Active/Standby 노드 쌍 + 서비스 + 정책의 독립 관리 단위 |
| VIP | Virtual IP. Failover 시 Standby로 이동하는 서비스 대표 IP |
| Split-Brain | 양 노드가 서로를 장애로 오판하여 동시에 Active가 되는 상태 |
| 핑퐁(Ping-Pong) | 간헐 장애로 Failover가 반복 왕복하는 현상. 유예 시간(180초)으로 방지 |
| Runbook | 기동/종료/점검 절차서. 단계별 자동 실행 및 감사 로그 기록 |
| Self-Healing | 장애 노드를 AI가 진단하고 레이어별 복구 스크립트로 자동 복구하는 기능 |
| MTTR | Mean Time To Repair. 장애 발생부터 서비스 정상화까지 평균 시간 |

---

*문서 끝. (v1.1 — 2026-06-11 데이터 모델·릴리스 범위·부록 보강)*
